...
const LearningAreasSection = () => {
  ...
  const renderRow = (rowNumber, direction = 'left') => {
    ...
    const triplicatedItems = [...items, ...items, ...items]; // Triple cloning for seamless loop
    
    // Calculate total width for proper sizing
    const itemWidth = 220; // 200px min-width + margins
    ...
    return (
      <div className="overflow-hidden relative w-full py-3 group" style={{ WebkitMaskImage: "linear-gradient(to right, transparent 0%, black 5%, black 95%, transparent 100%)", maskImage: "linear-gradient(to right, transparent 0%, black 5%, black 95%, transparent 100%)" }}>
        ...
        >
          {triplicatedItems.map((topic, index) => (
            <div
              key={`${topic.text}-${index}-${rowNumber}`} 
              className={`flex items-center justify-center p-4 px-6 rounded-lg shadow-md mx-2 flex-shrink-0 ${topic.color} whitespace-nowrap hover:scale-105 hover:shadow-lg transition-all duration-300`}
  ...
  return (
    <section ref={sectionRef} id="aprendizado" className="py-16 md:py-24 bg-background">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12 md:mb-16 text-left max-w-3xl">
           <AnimatedTitle 
            text={titleText}
            el="h2"
            className="text-3xl md:text-4xl lg:text-4xl font-bold leading-tight mb-4 text-foreground"
          ...
          <AnimatedTitle 
            text={subtitleText}
            el="h2" 
            className="text-2xl md:text-3xl lg:text-3xl font-semibold leading-tight text-muted-foreground mb-2 sm:mb-0"
            delay={0.5} 
            stagger={0.02}
          />
        </div>
